...
struct SuggestionsGateway {
    ...
    func getSuggestions(for hour: Int, on day: Date) -> [Suggestion] {
        var suggestions = [Suggestion]()
        
        // Get all possible suggestions
        suggestions.append(contentsOf: getFrequentlyAdded(for: hour, on: day))
    ...
    private func getFrequentlyAdded(for hour: Int, on day: Date) -> [Suggestion] {
        let lastMonthsBlocks = DataGateway.shared.getLastMonthsHourBlocks(from: day, for: hour)
